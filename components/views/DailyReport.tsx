...
/* ══════════════════════════════════════════
   Blob URL 印刷（Excel 工事打合簿 準拠レイアウト）
══════════════════════════════════════════ */
function buildPrintHtml(r: DailyReport, wsName: string): string {
  ...
  const fmDate = (d: string) => {
    ...
    const dt = new Date(d);
    return `20${y.slice(2)}年${String(Number(m))}月${String(Number(day))}日（${DAYS[dt.getDay()]}）曜日`;
  };

  // ─── スタイル定数 ───
  const B  = "border:1px solid #000;";
  const H  = `${B}background:#d9d9d9;font-weight:bold;text-align:center;padding:2px 4px;`;
  const D  = `${B}padding:2px 4px;vertical-align:middle;`;
  const DT = `${B}padding:2px 4px;vertical-align:top;`;

  // ─── 業者テーブル（Excel 行6-17：各業者2行、列 業者名/職種/作業内容/機械名称/台数/累計）───
  const subs = r.subcontractors.length
    ? r.subcontractors
    : Array(3).fill(null).map(() => ({ company: "", jobType: "", workContent: "", machineName: "", machineCount: 0, machineCumCount: 0, workers: 0, startTime: "", endTime: "" }));

  const subRows = subs.map(s => `
    <tr style="height:22px">
      <td rowspan="2" style="${D}text-align:center">${esc(s.company)}</td>
      <td rowspan="2" style="${D}text-align:center">${esc(s.jobType)}</td>
      <td rowspan="2" style="${DT}">${esc(s.workContent)}</td>
      <td style="${D}">${esc(s.machineName)}</td>
      <td style="${D}text-align:center">${s.machineCount || ""}</td>
      <td style="${D}text-align:center">${s.machineCumCount || ""}</td>
    </tr>
  ...
  // ─── 就労人員（Excel 行21-29）───
  const emps = r.attendees.length ? r.attendees
    : Array(5).fill(null).map(() => ({ name: "", jobType: "", present: true, startTime: "", endTime: "" }));
  const subEmps = r.subcontractors.length ? r.subcontractors
    : Array(5).fill(null).map(() => ({ company: "", jobType: "", workers: 0, startTime: "", endTime: "" }));
  const maxRows = Math.max(emps.length, subEmps.length, 5);
  ...
  const laborRows = Array.from({ length: maxRows }, (_, i) => {
    const a = emps[i] ?? { name: "", jobType: "", present: true, startTime: "", endTime: "" };
    const s = subEmps[i] ?? { company: "", jobType: "", workers: 0, startTime: "", endTime: "" };
    return `<tr style="height:18px">
      <td style="${D}">${esc(a.name)}</td>
      <td style="${D}text-align:center">${esc(a.jobType)}</td>
      <td style="${D}text-align:center">${a.name ? esc(a.startTime) + "～" + esc(a.endTime) : "　　　　　　～"}</td>
      <td style="${D}">${esc(s.company)}</td>
      <td style="${D}text-align:center">${s.company ? (s.workers || "") : ""}</td>
      <td style="${D}text-align:center">${esc(s.jobType)}</td>
      <td style="${D}text-align:center">${s.company ? esc(s.startTime) + "～" + esc(s.endTime) : "　　　　　　～"}</td>
    </tr>`;
  }).join("");

  // ─── 現場出来高（Excel 行30-38：CASEが列、全数量/本日等が行 ← 転置形式）───
  const pItems = r.progressItems?.length ? r.progressItems : [
    { caseType: "CASE-A",   unit: "本", totalQty: 0, todayQty: 0, cumQty: 0, remainQty: 0, progress: 0 },
    { caseType: "CASE-B",   unit: "本", totalQty: 0, todayQty: 0, cumQty: 0, remainQty: 0, progress: 0 },
    { caseType: "CASE-C",   unit: "本", totalQty: 0, todayQty: 0, cumQty: 0, remainQty: 0, progress: 0 },
    { caseType: "簡易土留部", unit: "本", totalQty: 0, todayQty: 0, cumQty: 0, remainQty: 0, progress: 0 },
  ];

  const pCol = (fn: (p: typeof pItems[0]) => string | number) =>
    pItems.map(p => `<td style="${D}text-align:center">${esc(fn(p))}</td>`).join("");

  const progressInnerTable = `
    <table style="width:100%;border-collapse:collapse;table-layout:fixed">
      <tr>
        <td style="${H}width:18%">工種/種類</td>
        ${pItems.map(p => `<td style="${H}">${esc(p.caseType)}</td>`).join("")}
      </tr>
      <tr>
        <td style="${H}">（単位）</td>
        ${pCol(p => p.unit)}
      ...
      <tr>
        <td style="${H}">進捗率</td>
        ${pCol(p => p.progress ? p.progress + "%" : "")}
      </tr>
    </table>`;

  // ─── 承認欄 ───
  const CONFIRM_ROLES = ["現場代理人", "主任技術者", "監理技術者", "発注者確認"];

  const signTable = (roles: string[]) => `
    <table style="width:100%;border-collapse:collapse;table-layout:fixed">
      <tr>${roles.map(role => `<td style="${H}font-size:9px">${esc(role)}</td>`).join("")}</tr>
      <tr style="height:52px">${roles.map(role =>
        `<td style="${D}font-size:14px;text-align:center">${r.approvals?.[role] ? "✓" : ""}</td>`
      ).join("")}</tr>
    </table>`;

  return `<!DOCTYPE html><html lang="ja"><head><meta charset="utf-8">
<title>工事打合簿 ${esc(r.implementDate)} ${esc(wsName)}</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:"ＭＳ ゴシック","MS Gothic","Yu Gothic",monospace;font-size:10px;color:#000;background:#fff}
  h1{font-size:12.5px;text-align:center;padding:3px 0;letter-spacing:0.15em;font-weight:bold}
  table{width:100%;border-collapse:collapse;table-layout:fixed}
  .pre{white-space:pre-wrap}
  @media print{@page{size:A4 portrait;margin:7mm 9mm}body{font-size:9.5px}}
</style></head><body>
...
<table style="margin-bottom:1px">
  <colgroup>
    <col style="width:10%"><col style="width:2%"><col style="width:20%">
    <col style="width:10%"><col style="width:2%"><col>
  </colgroup>
  <tr>
    <td style="${H}">工事番号</td><td style="${H}text-align:center">：</td><td style="${D}">${esc(r.workspaceId ?? "")}</td>
    <td style="${H}">工事略称</td><td style="${H}text-align:center">：</td><td style="${D}font-weight:bold">${esc(wsName)}</td>
  </tr>
  <tr>
    <td style="${H}">打合日</td><td style="${H}text-align:center">：</td><td style="${D}">${esc(fmDate(r.meetingDate))}</td>
    <td style="${H}">実施日</td><td style="${H}text-align:center">：</td><td style="${D}">${esc(fmDate(r.implementDate))}</td>
  </tr>
  <tr>
    <td style="${H}">天気</td><td style="${H}text-align:center">：</td>
    <td colspan="4" style="${D}">　晴　・　雨　・　曇り　（　${esc(r.weather)}　）</td>
  </tr>
</table>
...
<table style="margin-bottom:1px">
  <colgroup>
    <col style="width:18%"><col style="width:10%"><col><col style="width:18%"><col style="width:7%"><col style="width:7%">
  </colgroup>
  <tr>
    <td rowspan="2" style="${H}">業　者　名</td>
    <td rowspan="2" style="${H}">職　種</td>
    <td rowspan="2" style="${H}">作　業　内　容</td>
    <td colspan="3" style="${H}">使　用　機　械</td>
  </tr>
  <tr>
    <td style="${H}">名　称</td>
    <td style="${H}">台数</td>
...
<table style="margin-bottom:1px">
  ...
  <tr>
    <td style="${H}">作業指示・安全衛生指示等</td>
    <td style="${H}">確認・是正指示事項</td>
  </tr>
  <tr>
    <td class="pre" style="${DT}height:56px;padding:3px 4px">${esc(r.plannedWork)}</td>
    <td class="pre" style="${DT}height:56px;padding:3px 4px">${esc(r.actualWork)}</td>
  </tr>
</table>
...
<table style="margin-bottom:1px">
  <colgroup>
    <col style="width:18%"><col style="width:8%"><col style="width:17%">
    <col style="width:17%"><col style="width:6%"><col style="width:8%"><col style="width:14%">
  </colgroup>
  <tr><td colspan="7" style="${H}">就　労　人　員　・　集　計</td></tr>
  <tr>
    <td style="${H}">氏　　　名</td>
    <td style="${H}">職種</td>
    <td style="${H}">就業時間</td>
  ...
  <tr style="height:16px">
    ...
    <td style="${D}text-align:center">～</td>
    <td style="${H}text-align:center">社　員</td>
    <td colspan="2" style="${H}text-align:center">協力業者</td>
    <td style="${H}text-align:center">計</td>
  </tr>
  <tr style="height:16px">
    <td style="${D}"></td><td style="${D}"></td>
    <td style="${D}text-align:center">～</td>
    <td style="${D}text-align:center;font-weight:bold">${empTotal}　名</td>
    <td colspan="2" style="${D}text-align:center;font-weight:bold">${subTotal}　名</td>
    <td style="${D}text-align:center;font-weight:bold">${empTotal + subTotal}　名</td>
  </tr>
  <tr style="height:16px">
    <td style="${D}"></td><td style="${D}"></td>
    <td style="${D}text-align:center">～</td>
    <td style="${D}text-align:center">　　　　h</td>
    <td colspan="2" style="${D}text-align:center">　　　　h</td>
    <td style="${D}text-align:center">　　　　h</td>
  </tr>
</table>

<!-- 行30-38: 現場出来高（左55%）+ 記事（右45%） -->
<table style="margin-bottom:1px">
  <colgroup><col style="width:57%"><col></colgroup>
  <tr>
    <td style="${H}">現　場　出　来　高</td>
    <td style="${H}">記　事</td>
  </tr>
  <tr>
    <td style="padding:0;vertical-align:top">${progressInnerTable}</td>
    <td style="${DT}vertical-align:top;padding:3px 4px">
      <div class="pre" style="min-height:80px">${esc(r.notes)}</div>
      <div style="border-top:1px solid #000;margin-top:4px;padding-top:3px">
        ・無災害継続時間　：　${esc(r.disasterFreeHours ?? "")}　ｈ
      </div>
    </td>
  </tr>
</table>
...
<table>
  ...
  <tr style="padding:0">
    <td style="padding:0">${signTable(APPROVAL_ROLES)}</td>
    <td style="padding:0">${signTable(CONFIRM_ROLES)}</td>
  </tr>
</table>
