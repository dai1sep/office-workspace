    ...
    <div className="bulletin-view" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <Modal open={Boolean(detail)} onClose={() => setDetailId(null)} title="掲示の詳細" width={780}>{detail && <div style={{ display: "grid", gap: 14 }}>
        ...
        {detail.allowReactions && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, padding: "12px 14px", background: "var(--soft)", borderRadius: 8 }}>
            <div>
              <div className="muted-text" style={{ fontSize: 11, marginBottom: 6 }}>確認済 {detail.reactions.length}名</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{detail.reactions.map((id) => <span key={id} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, color: "var(--green)" }}><span style={{ width: 18, height: 18, borderRadius: "50%", background: "var(--green)", color: "white", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700 }}>{userName(state, id)[0]}</span>{userName(state, id)}</span>)}{detail.reactions.length === 0 && <span className="muted-text" style={{ fontSize: 12 }}>まだいません</span>}</div>
            </div>
            <div>
              <div className="muted-text" style={{ fontSize: 11, marginBottom: 6 }}>未確認 {state.users.filter((u) => !detail.reactions.includes(u.id)).length}名</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{state.users.filter((u) => !detail.reactions.includes(u.id)).map((u) => <span key={u.id} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, color: "var(--muted)" }}><span style={{ width: 18, height: 18, borderRadius: "50%", background: "var(--line)", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 10 }}>{u.name[0]}</span>{u.name}</span>)}{state.users.every((u) => detail.reactions.includes(u.id)) && <span style={{ fontSize: 12, color: "var(--green)" }}>全員確認済</span>}</div>
            </div>
          </div>
        )}
